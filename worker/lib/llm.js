...
async function callLlmDigest(env, items) {
  ...
  const prompt =
    ...
    '{"headline":"короткий заголовок выпуска (1 фраза с позицией)", "bullets":["ровно по 1 булету на каждую новость (до ~150 символов) по формуле «факт — вердикт TrustNode»"], "advice":["не более 2 советов, каждый до ~50 символов"]}.\n' +
    "Вердикт — это ОБЯЗАТЕЛЬНО выраженное отношение редакции: предостережение, оценка, ирония, вывод «что это значит для читателя». Это НЕ пересказ заголовка и НЕ добавление слов «важно/бдительность», а позиция автора.\n" +
    "Пример эталона: новость «Выявлена схема мошенничества при покупке машин за рубежом» → булет: «Продавцы машин за рубежом выманивают предоплату и исчезают. Наш вердикт: если требуют задаток до осмотра — это развод, не ведитесь.»\n" +
