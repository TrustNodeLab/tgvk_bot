...
async function callLlmDigest(env, items) {
  ...
  const prompt =
    ...
    '{"headline":"короткий заголовок выпуска (1 фраза)", "bullets":["по каждой новости РОВНО 2-3 предложения, как диктор новостей: что произошло, какие цифры, почему это касается читателя"], "advice":["2-3 совета, как защититься"]}.\n' +
    "Не ужимай до одного предложения и не пиши один абзац за новость; вещательный тон ведущего, без канцелярита; суммы — точно из текста новостей.\n\n" +
    list;
