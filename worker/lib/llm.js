...
async function callLlmDigest(env, items) {
  ...
  const list = items
    ...
    .join("\n\n");
  const prompt =
    "Ты — автор канала TrustNode о кибербезопасности. По списку новостей собери живой дайджест:\n" +
    "верни ТОЛЬКО валидный JSON без пояснений:\n" +
    '{"headline":"короткий заголовок выпуска (1 фраза с интригой или позицией)", "bullets":["ровно по 1 булету на каждую новость (до ~150 символов): факт + твоё живое мнение о нём"], "advice":["не более 2 советов, каждый до ~50 символов"]}.\n' +
    "Мнение — это НЕ слово «важно» и НЕ пересказ заголовка. Это как человек рассказывает знакомому: эмоция, ирония, сарказм, предостережение, взгляд «что здесь не так» или «что это значит для нас».\n" +
    "Примеры эталонов:\n" +
    "— новость «Выявлена схема мошенничества при покупке машин за рубежом» → «Продавцы авто за границей берут задаток и испаряются. Как же знакомо: хочешь выгодно — рискуешь остаться и без машины, и без денег.»\n" +
    "— новость «Мошенники наживаются на поиске удалённой работы» → «Работа мечты из дома? Только сначала оплати доступ к вакансии. Спойлер: это ловушка, таких „работодателей“ надо слать лесом.»\n" +
