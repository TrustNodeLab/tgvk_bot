...
SHORT_SCRIPT_SYSTEM_PROMPT = (
    ...
    "сценарий озвучки на 30-60 секунд (от 80 до 160 слов), который диктор "
    "читает вслух.\n"
    "Требования:\n"
    "1. 4-6 секций, каждая — заголовок с новой строки, начинается с символа "
    "# (например: `# Держись подальше от QR-кодов`), ниже — абзац текста.\n"
    "2. Заголовок цепляет, текст секции — живой разговорный русский без "
    "канцелярита: конкретика, цифры, детали из источника, короткие предложения.\n"
    "3. Первая секция — крюк: интрига, вопрос или шокирующий факт за 1-2 "
    "предложения.\n"
    "4. Последняя секция — вывод-рекомендация в одну строку (как подписывают "
    "эксперты: «Проверьте свои настройки прямо сейчас»).\n"
    "5. Запрещено: «важно/актуально/стоит отметить», общие фразы, вступления "
    "вроде «в этой статье мы расскажем», ссылки на источник в тексте, "
    "markdown-жирное.\n"
...
def rewrite_post_to_script(post_text: str, mode: str = "short",
    ...
    mode="short" — TikTok-шортс 30-60 сек (4-6 секций с # заголовками);
    mode="long"  — YouTube-ролик 5-7 мин (6-7 секций).

    Возвращает текст сценария в формате parse_script / video_long:
    строки вида "# Заголовок" + абзацы через пустую строку.
