...
async function handleCommand(env, state, chatId, text, msg) {
  ...
  switch (cmd) {
    ...
    case "/cine": {
      ...
      const reply = (msg && msg.reply_to_message) || null;
      // M18: статья прямо в команде (длинный текст после /cine) — тоже сценарий
      const inlineArticle = !reply && rawArgs.length > 200;
      const script = reply
        ? String(reply.text || reply.caption || "").trim().slice(0, 3500)
        : (inlineArticle ? rawArgs.slice(0, 3500) : "");
      let topic = inlineArticle ? "" : rawArgs.slice(0, 300);
      if (!topic && script) {
        // инлайн: воркер уже схлопнул переводы строк в args — тема =
        // первое предложение; реплай: первая строка (переводы целы)
        topic = (inlineArticle
          ? script.split(/[.!?…]\s/)[0]
      ...
      if (!topic) {
        await sendMessage(env, chatId,
          ...
          `Со статьёй: текст после команды или ответом на статью — соберу трейлер ПО ТЕКСТУ статьи и пришлю сюда же (~5–8 мин).`);
        break;
      }
      try {
        ...
        if (r.ok) {
          ...
          await sendMessage(env, chatId,
            `⚠️ Cinematic недоступен: в воркере нет GITHUB_TOKEN/OWNER/REPO. ` +
            `Добавьте секреты и задеплойте воркер заново.`);
        } else {
          await sendMessage(env, chatId, `⚠️ Не смог запустить cinematic: ${escHtml(r.reason || "ошибка")}`);
        }
      } catch (e) {
